...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        pre_nms_limit = tf.minimum(self.config.RPN_PRE_NMS_LIMIT,
                                   tf.shape(anchors)[1])
...
class PyramidROIAlign(KE.Layer):
    ...
    - image_shape: [batch, (shape)] Image details
...
class PyramidROIExtract(KE.Layer):
    """Implements ROI Pooling on level 2 of the feature pyramid.
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        mrcnn_bbox = inputs[2]
        window_np = np.array([[0., 0., 1., 1.]]*self.config.BATCH_SIZE)
        window = K.variable(value=window_np,
                            dtype='float32', name='det_window')
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, config):
    ...
    # TODO: To hard example mine or not to hard example mine, that's the
    # question
...
def data_generator(dataset, config, shuffle=True, augment=False,
                   augmentation=None, random_rois=0, batch_size=1,
    ...
    augmentation: Optional. An imgaug (https://github.com/aleju/imgaug) augmentation.
        For example, passing imgaug.augmenters.Fliplr(0.5) flips
        images right/left 50% of the time.
    random_rois: If > 0 then generate proposals to be used to train the
                 network classifier heads. Useful if training
        ...
        try:
            ...
            if random_rois:
                ...
                if detection_targets:
                    rois_shape = (config.TRAIN_ROIS_PER_IMAGE, 4)
                    batch_rois = np.zeros(
                        (batch_size,) + rois_shape, dtype=np.int32)
                    batch_mrcnn_class_ids = np.zeros(
                        (batch_size,) + (config.TRAIN_ROIS_PER_IMAGE),
                        dtype=np.int32)
                    mrcnn_bbox_shape = (
            ...
            for b in range(batch_size):
                ...
                if random_rois:
                    rpn_rois = generate_random_rois(
                        images[b].shape, random_rois,
                        gt_class_ids[b], gt_boxes[b])
                    if detection_targets:
                        rois, mrcnn_class_ids, mrcnn_bbox =\
                            build_detection_targets(
                                rpn_rois[b], gt_class_ids[b],
                                gt_boxes[b], config)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        input_image = KL.Input(
            shape=[None, None, bands], name="input_image")
        input_image_shape = KL.Input(shape=[3],
                                     name="input_image_shape")
        if mode == "training":
            # RPN GT
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        augmentation: Optional. An imgaug (https://github.com/aleju/imgaug)
            ...
            with the keras fit_generator method. Must be list of type
            keras.callbacks.
            no_augmentation_sources: Optional. List of sources to exclude for
            augmentation. A source is string that identifies a dataset and is
            defined in the Dataset class.
    ...
    def unmold_detections(self, detections, image_shape):
        ...
        detections: [N, (y1, x1, y2, x2, class_id, score)] in normalized
            coordinates
    ...
    def detect(self, images, verbose=0):
        ...
        image_shape = np.array([images[0].shape]*self.config.BATCH_SIZE,
                               dtype=np.int32)
        # Anchors
        anchors = self.get_anchors(image_shape[0][:2])
        # Duplicate across the batch dimension because Keras requires it
        # TODO: can this be optimized to avoid duplicating the anchors?
        anchors = np.broadcast_to(anchors,
        ...
        if verbose:
            log("image_shape", image_shape)
            log("anchors", anchors)
        # Run object detection
        detections, _, _, _, _, _ =\
            self.keras_model.predict([images, image_shape, anchors],
                                     verbose=0)
        # Process detections
        results = []
    ...
    def run_graph(self, images, outputs):
        ...
        anchors = np.broadcast_to(anchors,
                                  (self.config.BATCH_SIZE,) + anchors.shape)
        model_in = [images, image_shape, anchors]
