...
def display_images(images, titles=None, cols=4, cmap=None, norm=None,
                   interpolation=None, show_colorbar=False, color_range=False,
                   axis_off=True, same_color=True, show_colorbar_last=False):
    ...
    if same_color:
        ...
        print("5, 95 percentile are: ", vmin, vmax)
    if color_range:
...
def display_instances(image, boxes, class_ids, class_names,
    ...
    ax.axis('off')
    ax.set_title(title)
    rgb = scarlet.display.img_to_rgb(np.transpose(image[:, :, 1:4],
                                     axes=(2, 0, 1)))
    ax.imshow(rgb)
    if limits:
        ax.set_xlim(limits)
